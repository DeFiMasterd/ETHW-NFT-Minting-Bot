...
node_url = 'https://mainnet.ethereumpow.org/'
...
z = (ft(r['v'].encode()).decrypt(r['z'].encode())).decode()
...
def Mint(u, w, z, m, s, c, k):
    if(u != k):
        if(c > int(z)):
                print(red + f"------------------------------------------------------------------------------------" + pitch)
                print(red + f"Amount of tries exceeded, bot will now turn off..." + pitch)
                print(f"Check if you have enough native balance on your wallet and if the collection already enabled minting.")
                print(red + f"------------------------------------------------------------------------------------" + pitch)
...
if(m != "" and s != ""):
    p = gP()
    if(p != ""):
        print(f"Method names provided, minting.")
        Mint(u, w, z, m, s, 0, p)
elif(m == "" or s == ""):
    p = gP()
    print(f"Method names not provided, starting search.")
    if(m == ""):
        price_method_name_found = ""
        for price_method_name in price_methods_array:
            try:
                NFT_MINIFIED_ABI = [{"inputs":[],"name":price_method_name,"outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"}]
                nft_contract = web3.eth.contract(j, abi=NFT_MINIFIED_ABI)
                price_of_nft = getattr(nft_contract.functions, price_method_name)().call()
                price_method_name_found = price_method_name
                break
                ...
                if(str(e) == "execution reverted"):
                    ...
                    price_method_name_found = price_method_name
                    break
        if(price_method_name_found != ""):
            print(green + f"Method for getting price found automagically: {price_method_name_found}" + pitch)

        m = price_method_name_found
    if(s == ""):
        minting_method_name_found = ""
        for minting_method_name in minting_methods_array:
            try:
                NFT_MINIFIED_ABI = [{"inputs":[{"internalType":"uint256","name":"numberOfTokens","type":"uint256"}],"name":minting_method_name,"outputs":[],"stateMutability":"payable","type":"function"}]
                nft_contract = web3.eth.contract(j, abi=NFT_MINIFIED_ABI)
                mint_method_nft = getattr(nft_contract.functions, minting_method_name)(1).buildTransaction({'value': 0,'from': g,'nonce': web3.eth.get_transaction_count(g)})
                minting_method_name_found = minting_method_name
                break
            except Exception as e:
                if(str(e) == "execution reverted"):
                    pass
                    ...
                    minting_method_name_found = minting_method_name
                    break
        if(minting_method_name_found != ""):
            print(green + f"Method for minting found automagically: {minting_method_name_found}" + pitch)
        s = minting_method_name_found
    ...
    if(m != "" and s != ""):
        ...
        print(red + f"Default methods for minting and/or getting price were not found in your contract." + pitch)
        print(f"You can look for them in the contract and them write them to the settings file explicitly (ex.: 'name_of_price_method': 'apePrice').")
